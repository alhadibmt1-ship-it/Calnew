...
export default function BoatLoanCalculator() {
  ...
  const calculate = () => {
    const s = values;
    try {
      const loan=(parseFloat(s.price)||0)-(parseFloat(s.downPmt)||0);const r=(parseFloat(s.rate)||0)/1200;const n=(parseFloat(s.term)||10)*12;
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto" data-testid="boat-loan-calculator">
      <CardHeader>
        <CardTitle className="text-2xl">Boat Loan Calculator</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-medium mb-1" htmlFor="price">Boat Price ($)</label>
              <Input
                id="price"
